...
- [ ] *(from Phase 4 QA, 2026-08-01)* A session whose `title` is the **empty string** renders a usable placeholder, not a blank row. Measured on the real store: **185 of 1,210 sessions (15.3%)** have neither a title nor a first prompt to fall back to; a 125-row census returned 23 blank titles (18.4%). The endpoint is correct — it has nothing to send — so the placeholder is the rail's job
- [ ] *(from Phase 4 QA, 2026-08-01)* A session whose **workspace directory no longer exists** is distinguishable in the rail. Measured: **14 of 65 workspaces** name a directory that is gone, including the 208-session `nrf_tool` worktree that is D19's own showcase. Those rows report `available` because D17 measures **lock liveness, not directory existence** — correct per contract, but the rail would otherwise offer sessions that fail at resume with no warning
