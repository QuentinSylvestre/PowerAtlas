...
- [x] All 7 ripple-effect call sites (§1) repointed to the renamed sole supervisor by name, no `None`-guards added (none needed under eager construction) — additionally, ~12 dead `if _supervisor is None: ... return` guards inside individual handler functions (left over from lazy-construction-era code, beyond the 7 named sites) were found and removed for consistency with D1's "no guards needed anywhere"
