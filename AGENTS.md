...
- **Never restart PowerAtlas autonomously** unless the user grants restart for the current task; a grant covers that task only. Restarting kills every kiro-cli ACP process the supervisor is managing, terminating all active sessions. Without a grant, defer to the user — present the restart as needed and let them do it when ready.
