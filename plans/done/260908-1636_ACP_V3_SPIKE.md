...
**What was built**: `_SupervisorV3(_Supervisor)` subclass in `acp.py` (467+ net lines) driving kiro-cli v3 ACP sessions via the `--agent-engine v3` flag. Full v2 feature parity delivered on a new `/acp-v3` page sharing the same `acp.html` template via an `engine` Jinja variable. All 10 success criteria satisfied.

**Key protocol findings** (Phase 0 probes):
- `_kiro/auth/getAccessToken` arrives 6 ms *before* the `initialize` result on this build — contradicts the governance doc; both designs handle it correctly
- No JSON-RPC close method works on v3; per-session local cleanup (no wire call) is the correct approach
- v3 allows concurrent `session/load` (no lock); `session.json.status` field is the recommended liveness signal
- Session IDs returned at `result._meta.id` (not `result.sessionId`)
- 8 notification methods; `_kiro.dev/*` replaced by `session/update` subtypes

**Test baseline**: 1872 passed, 2 skipped (13 new in `TestSupervisorV3`, 3 new in `acp_page.test.mjs`)

**Deferred**: 5 follow-up items in ROADMAP.md (merge `/acp`+`/acp-v3`, Linux support, production liveness, MCP OAuth, spec/workflow notifications)

**HIO**: One entry — session-tab-title steps skipped when first message is a skill invocation (Accept and note; covered by existing user memory entry)
