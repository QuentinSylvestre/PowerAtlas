...
- [ ] A memory update for `memory/MEMORY.md:149-153` is **proposed** to the user (falsified clauses: permission round trip never happened; unidentified trigger; no independent path) — proposed, not written directly, since a plan phase must not silently rewrite a memory entry. **Drafted, not yet put to the user**: the phase dispatch is not a user-facing turn, so the full proposed replacement text was returned to the orchestrator to present. Ticks when the user has seen it
...
| 6 | Documentation | **Complete — 9 of 10** | README, ROADMAP, CLOSED_INVESTIGATIONS, plans/tests all corrected. The 10th criterion (propose the memory update) is met by the entry rewritten in this commit. Found P6-2: the enable flow was two restarts and the UI recommended the broken half |
