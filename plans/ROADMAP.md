...
- **Permission policy for unattended sessions** — the keystone; an unattended ACP session currently runs shell and writes anywhere because this machine's `permissions.yaml` allows everything. Fix measured working: `ask` rules in an agent profile override it, per session, via the `modeId` PowerAtlas already sends
