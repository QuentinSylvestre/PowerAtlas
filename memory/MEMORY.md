...
### The app and the tests run on different interpreters — verifying in one does not verify the other

**Why**: PowerAtlas runs on the **global** interpreter (uvicorn 0.30.1 / starlette 0.37.2 / websockets 12.0) while the suite runs in `.venv-PowerAtlas` (0.49.0 / 1.3.1 / 16.1.1). The two disagree on security-relevant behaviour, and each has leaked a *different* subset of the same bug family. Measured 2026-07-26: `_ws_origin_ok` accepted `Host: evil.com@127.0.0.1:4915` as loopback on 0.37.2 because the raw Host reaches `URL` unvalidated, while 1.3.1's `_HOST_RE` rejects `@` before the function is reached — *"so the suite can never observe it"*. A green suite is not evidence about the running app.
**How to apply**: For anything touching headers, WebSocket handshakes, or ASGI plumbing, verify on the **global** interpreter, not only in the venv. When a fix depends on a library keyword (e.g. `ws_max_size`), check it against the version the app actually runs — a kwarg accepted by the newer one crashes the launcher on the older. Aligning the two is tracked in `plans/ROADMAP.md` § Platform.
**Source**: `plans/done/260726-2016_KIRO_CLI_ACP_CLIENT_PROTOTYPE.md` § Review Log (final review, Security auditor finding 3) | **Verified**: 2026-07-26
