...
- **Permission policy for ACP sessions** *(scope settled to interactive, planned 2026-09-21)* — an ACP session currently runs shell and writes anywhere because this machine's `permissions.yaml` allows everything. Fix measured working: `ask` rules in an agent profile override it, per session, via the `modeId` PowerAtlas already sends. Paired with the loopback credential below
...
### Misc
- **[SECURITY] Loopback API token** — any local process can create sessions and run shell commands via `/api/*`; the "inject the secret into the page" shape was corrected 2026-09-21 (a local GET reads it straight back out) — now a one-time login code exchanged for an HttpOnly cookie, planned together with the permission item
...
| # | Item | Tier | Why here |
|---|---|---|---|
| 1 | *Permission policy for unattended sessions* | **keystone** | Gates all six Automation items, and as of 2026-09-19 the mechanism is measured working rather than unknown: an `ask` rule in an **agent profile** overrides the machine's allow-all user file, because permission scopes merge most-restrictive-wins. PowerAtlas already selects the agent per session via `modeId`, so this is a config artifact plus a picker entry, not a new subsystem. Also the cheapest mitigation for item 2 |
| 2 | *[SECURITY] Loopback API token* | days | **Promoted 2026-09-19 on measured evidence.** An ACP session today executes shell and writes arbitrary absolute paths with no gate, so an unauthenticated loopback API composes into arbitrary code execution as the user for any local process. Ranked below the keystone only because the keystone now removes the *ungated* half cheaply; this removes the *unauthenticated* half, and both are wanted. Nothing here waits on anything external |
...
- **Permission policy for unattended sessions** — the keystone. Decide `session/request_permission` by rule when nobody is watching, gated at the tool boundary rather than the prompt.
  - *Premise, corrected three times — read the whole sequence before designing anything.* It began as "drop `-a` and decide each request automatically". Corrected 2026-09-19 to "`-a` is gone and every request waits for a human, so an unattended session **stalls**". Measurement falsified that: an ACP session under the shipped `kiro_default` ran shell, read, write and an **out-of-cwd absolute-path write** with `session/request_permission` raised **zero times across 5 sessions**. A first reading of that concluded no gate exists on `acp`+v3 — **that was wrong too**, and the next bullet says why. The durable statement is narrower and is what matters: **as this machine is configured today, an unattended ACP session executes shell commands and writes anywhere on disk with nothing watching.** That is a configuration fact, not a protocol limit, and it is fixable from inside PowerAtlas.
  - *Why nothing prompts — found 2026-09-19, and it is our own configuration.* `~/.kiro/settings/permissions.yaml` on this machine reads `rules: [{capability: all, effect: allow}]`, commented "equivalent of `--trust-all-tools` in v2". It is a tracked `seed-file` in the agent-playbook inventory, deployed by `setup.ps1`. **kiro-cli's documented default is the opposite** — `fs_read` on `./**`, read-only git and system info, with everything else prompting. So the permission mechanism was never missing; this machine turns it off globally, deliberately, for every kiro-cli session including ACP's.
  - *The permission model, from the docs.* Rules are `{capability, match?, exclude?, effect}` with `effect ∈ {allow, ask, deny}` and capabilities `fs_read`/`fs_write`/`shell`/`web_fetch`/`web_search`/`mcp`/`subagent`/`skill`/`power`/`context`/`diagnostics`/`sandbox_network` plus the meta-values `all`/`builtin`/`filesystem`. Six scopes contribute — Kiro (hardcoded), administration, **user** (`~/.kiro/settings/permissions.yaml`), **workspace** (`~/.kiro/workspace-roots/<hash>/permissions.yaml`), **agent** (embedded in the agent profile) and session. They **merge with no precedence between them: the most restrictive effect wins** (`deny > ask > allow`). ACP clients may additionally seed a session with `_meta.kiro.policyPreset` (`allow-all`, `edit-workspace`, `read-workspace`, `read-all`, `read-only-shell`, `dev-shell`) — but presets are **allow-only**, they remove prompts rather than add them, so a preset cannot tighten anything and is not the lever here.
  - **The design this yields, measured working 2026-09-19 against the unchanged allow-all user file.** Because the most restrictive scope wins, an `ask` rule embedded in an *agent profile* overrides the user file's blanket `allow` — for sessions using that agent only. A probe agent carrying `permissions: {rules: [{capability: shell, effect: ask}, {capability: fs_write, effect: ask}]}` produced two real `session/request_permission` frames (options `accept`/`reject`/`always-reject`, the shell one titled with the literal command `echo PROBE_SHELL_OK`); denying both left `tool_call` at `failed` with no side effect and a clean `end_turn`, while `fs_read` — which had no rule — still completed silently. **This is the whole keystone**: interactive kiro-cli keeps its permissive posture from the untouched user file, ACP sessions get gated by their agent profile, and PowerAtlas already chooses the agent per session through the `modeId` channel the task-mode picker ships. Different postures per user or per job are additional agent profiles, not new machinery.
  - *Two stale claims retired by that measurement.* (1) The former "what is settled" line cited a 2026-07-31 / kiro-cli 2.16.0 measurement that predates the v2→v3 cutover; the round-trip is now re-confirmed directly on 2.22.0 / KAS 0.66.0 under v3, so SC-9's inline UI has a live input once an agent profile asks for one. (2) `agent-playbook`'s `memory/topics/kiro-cli.md` records that an inline `permissions:` block broke agent loading during the v3 migration — **no longer true on 2.22.0**: the agent loaded and its rules took effect, verified by reading `modeId` back off `config_option_update`.
  - *CLI flags remain a dead end on this path, which is why the profile is the answer.* `acp --agent-engine v3` rejects `-a`, `--trust-tools` **and** `--agent` alike. The terminal launcher's `--trust-tools *` (`launcher.py:112`) works only because it rides the **`chat`** subcommand — so the two spawn paths have genuinely different postures, and the terminal one stays wide open regardless of anything done here.
  ...
  - **Superseded 2026-09-21 — three points in the seed note above are wrong; the exploration corrected them.** (1) The `inventory.tsv` row for `kiro_default.md` is **`template-file`**, not `copy-file`, and the file carries a `{{PLAYBOOK_ROOT}}` token that only `template-file` substitutes — a `copy-file` row would have shipped the literal token as a resource path, which fails open and silent. (2) The `MERGED: <name> (shared + kiro overlay)` machinery **cannot be reused**: it is hard-wired to walk `shared/skills/<name>/` and match against `providers/kiro/skills/<name>/`, and there is no `shared/agents/` to overlay against. (3) **agent-playbook is no longer the owner at all** — the derived agent is a PowerAtlas artifact, generated by PowerAtlas from a user-configurable base agent, because a product feature must not depend on a personal config repo's deploy pipeline. Full record: `plans/260921_ACP_PERMISSION_PROFILE_AND_LOOPBACK_CREDENTIAL.md`.
  - *What is not settled* — the deny-floor pattern list and the transcribed default rule set; both are scoped to a Phase 0 spike in the plan rather than left open here. The "is an unattended flag per session, per dispatch, or per agent definition" question moved wholesale to the follow-on item below, with the rest of the unattended design.
  - **Corrected 2026-09-21 — this item does *not* gate the Automation & Workflows section.** Scope was settled to **interactive** ACP sessions only, so an unattended session under an `ask` rule simply blocks until the 30-minute silence ceiling — a bounded wait, not a policy, and not something the six Automation items can build on. The gating role passes to *Decide permission requests by rule for unattended sessions* below. What remains true is the pairing: this item and the `[SECURITY]` loopback token item are complementary halves of one exposure (ungated agent / unauthenticated caller) and are planned together in `plans/260921_ACP_PERMISSION_PROFILE_AND_LOOPBACK_CREDENTIAL.md`.

- **Decide permission requests by rule for unattended sessions** — the real Automation keystone, split out of the item above on 2026-09-21. When nobody is watching, a `session/request_permission` must be *decided*, not parked: approve read-class tools, deny shell and write unless an explicit pattern allows them, and after a bounded wait cancel the turn and mark the session as needing attention rather than leaving it to the silence ceiling.
  - *Why it is separate* — the item above gives the agent a mouth; this one gives PowerAtlas an answer. Without it an unattended session still stalls, so **this** is what the six `## Automation & Workflows` items actually wait on.
  ...
  - *It also absorbs the lean dispatch agent* (below). Narrowing `resources:` is right for a dispatched narrow task and wrong for an interactive session, so that folding belongs here rather than in the interactive item above.
...
- **[SECURITY] Loopback API token — authenticate local callers to the `/api/*` and `/ws/acp` surfaces.** Today any local process that can reach `127.0.0.1:<port>` can create sessions, send prompts, delete sessions, and read the full session list, with no credential required. The `same_origin_guard` is CSRF defence only (POST-scoped, passes a loopback `Host` unconditionally) and `RemoteAccessGuard` is non-loopback only — neither checks whether the caller is PowerAtlas's own browser UI or a rogue script. Anything that reaches the API can create a session, send it a prompt and answer its own permission requests over `/ws/acp`, so it can execute arbitrary shell commands as the user; the inline permission prompt gates the person holding the page, not a script speaking the same protocol. (This sentence used to say `/acp` hardcodes `-a`; it does not, see the permission-policy item.) **Priority rises with the permission-policy item**: once a session can auto-approve, an unauthenticated loopback API in front of it is the sharpest surface on the machine.
  - *Proposed shape — **corrected 2026-09-21**, the original is quoted below because the correction is the point.* The original read: a secret at `%LOCALAPPDATA%\power-atlas\local-secret`, required as an `X-PowerAtlas-Token` header on `/api/*` and `/ws/acp`, with browser clients "receiving the token injected into the HTML at page-load time so they need no manual handling". **Two things are wrong with that.** (1) *Injecting the secret into the page is the hole it is meant to close* — this is exactly how `_ACP_TOKEN` works today (`web.py:1269`, rendered into both templates), and a local process simply issues `GET /` and reads it out of the HTML. Any scheme that puts the secret in a fetchable body inherits that. (2) *`/api/*` and `/ws/acp` is not the boundary* — the token is scraped from `GET /`, which is in neither set, and the `env` leak above sits on `/partials/*`, also in neither.
  - *Corrected shape*: the secret on disk stays (it is how a script or agent you deliberately authorise gets in), but the browser never receives it. Each door that opens the UI mints a **one-time login code** into the URL, exchanged once for an **HttpOnly cookie**; the gate is then default-deny across every route, exempting only the exchange route and `/static`. A browser arriving without the cookie cannot self-serve one, because a rogue local process is indistinguishable from a typed URL at the HTTP layer — the tray's "Copy login link" is the deliberate re-entry path. Planned in `plans/260921_ACP_PERMISSION_PROFILE_AND_LOOPBACK_CREDENTIAL.md`.
