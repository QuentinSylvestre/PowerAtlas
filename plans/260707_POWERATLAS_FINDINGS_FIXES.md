...
- **CSRF/Origin** — *resolved 2026-07-07*: user **accepted the mitigation** (127.0.0.1 bind + random port + Phase 5 L1 RCE fix). Recorded as a tracked out-of-plan follow-up (add an `Origin`/`Host` allowlist middleware) in the Risk Assessment table — revisit before configuring a static `port`.
...
| Security (out of plan scope): no CSRF/Origin check on command-executing POSTs (`/api/provider/save`, launch endpoints) | Localhost drive-by could set `default_args` then launch (mitigated by random port + 127.0.0.1 bind; the Phase 5 L1 fix closes the RCE, leaving only unwanted-args/auto-launch) | **User accepted the mitigation (2026-07-07).** TRACKED FOLLOW-UP (separate work, not this plan): add an `Origin`/`Host` allowlist middleware on POST endpoints — do it before/if a static `port` is ever configured (which removes the random-port mitigation). |
