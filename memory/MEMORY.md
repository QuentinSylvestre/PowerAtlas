...
**Update (2026-08-19, Phase 7 of `260819_ACP_V3_SPIKE`)**: `_Supervisor._publish_live()` now emits the union of both the v2 (`_supervisor.sessions`) and v3 (`_supervisor_v3.sessions`) session sets to the shared hook. The hook injection path through `web.py` (`acp.set_sessions_changed_hook`) is unchanged; the v3 session set is folded in inside `_publish_live` itself, keeping presence.py entirely unmodified.
...
**Why**: Phase 8 and Step 9 of `260908_ACP_V3_PRODUCTION_HARDENING` found `_handle_new_v3` (the connection that *creates* a session) missing the resend of 3 separate broadcast-only, meta-cached frame types (`history`, `commands`/`skills`, `contextPercent`/`turnActive`) that `_handle_subscribe_v3` (the *reconnect* path) already correctly resent — found and fixed in 3 separate passes because nothing checked "every cached-but-broadcast-only field" against both entry points at once. Evidence: "an available_commands_update landing in the SC-1 buffer window is replayed and cached into meta[\"commands\"]/[\"skills\"] before attach() runs above, so its own broadcast also reached zero subscribers."
