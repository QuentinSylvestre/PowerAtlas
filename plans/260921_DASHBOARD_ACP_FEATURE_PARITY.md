...
- **Live QA for Phases 1-2**: both performed. Phase 2's was initially blocked by a disconnected Chrome browser extension (across multiple orchestrator- and sub-agent-side retries); resolved by restarting Chrome (user-authorized; it had exited entirely, so nothing was lost) once Chrome itself, not just the extension, turned out to be the actual root cause. If Phases 3-6 hit the same disconnection again, restart Chrome first before assuming a deeper problem.
