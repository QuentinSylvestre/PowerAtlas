...
>
> **2026-09-12 update (`260911_ACP_V2_TO_V3_ENGINE_CUTOVER`)**: the five `_ACP_V3_*` constants named above no longer exist. Their v3 URL *values* were taken over by the original v2-named constants (`_ACP_PATH`, `_ACP_WS_PATH`, `_ACP_LISTING_PATH`, `_ACP_WORKSPACES_PATH`, `_ACP_DELETE_PATH`), which now serve the sole (v3-descended) engine at the v2-spelled URLs (`/acp`, `/ws/acp`, `/api/acp/sessions`, `/api/acp/workspaces`, `/api/acp/sessions/delete`). `/acp-v3` and its siblings no longer route anywhere. See section 2.27 below for the corresponding test-target update.
...
### 2.16 Lifespan: background refresh loop + ACP idle sweeper + ACP crash watchdog + ACP teardown
- **what**: `lifespan` owns **four** concerns since plan `260923_ACP_V3_SESSION_DELETE_WATCHDOG_MCP_STATUS`. Before the `yield` it starts `_background_refresh` (calls `refresh_stale_entries` every 30s in an asyncio task) and — behind an `if acp is not None` guard, so an ACP import failure disables `/acp` rather than stopping the app — the **ACP idle sweeper** and the **ACP crash watchdog** (5 s `proc.poll()` loop detecting kiro-cli.exe exits before the reader thread sees EOF). After the `yield` it cancels all three ACP tasks and awaits them in one `asyncio.gather(..., return_exceptions=True)` **inside the nested block, before** tearing down the ACP agent process tree, so nothing any task raises can skip the teardown.
...
### 2.27 v3 session delete endpoint (api_acp_v3_delete_sessions) [superseded, see note]
> **2026-09-12 update (`260911_ACP_V2_TO_V3_ENGINE_CUTOVER`)**: `api_acp_v3_delete_sessions` was renamed to `api_acp_delete_sessions` (Phase 2), taking over the identity 2.26 originally described — v2's own live-protocol `api_acp_delete_sessions` was deleted outright, not merged. There is now one delete endpoint, not two; this section's distinct-route framing is historical. The five `_ACP_V3_*` path constants named below no longer exist — their v3 *values* (`/acp`, `/ws/acp`, `/api/acp/sessions`, `/api/acp/workspaces`, `/api/acp/sessions/delete`) were taken over by the original v2-named constants (`_ACP_PATH`, `_ACP_WS_PATH`, `_ACP_LISTING_PATH`, `_ACP_WORKSPACES_PATH`, `_ACP_DELETE_PATH`), which is why 2.26 above (unedited, pre-dating this cutover) still describes the currently-correct route.
- **what** (historical, describes the pre-cutover parallel-engine state): POST /api/acp-v3/sessions/delete. Mirrors 2.26 for v3 sessions, and (2026-09-09, Phase 5 of 260908_ACP_V3_PRODUCTION_HARDENING) v3 deletion now works end-to-end: sess_-prefixed IDs route to `data_kiro_v3.delete_session` (whole-directory removal under `~/.kiro/sessions/<hash>/`, with a rename-to-staging-then-rmtree survival trick for a locked file, mirroring 2.26's v2 design). Workspace-level delete (`{cwd: ...}`) enumerates and deletes v2 AND v3 sessions for that workspace via `_acp_sessions_for_workspace(cwd, include_v3=True)`, with the held-session check unioned across both `/acp` and `/acp-v3`'s supervisors so a session open in either engine is refused, not just the one the request came through. The five new v3 paths (_ACP_V3_PATH, _ACP_V3_WS_PATH, _ACP_V3_LISTING_PATH, _ACP_V3_WORKSPACES_PATH, _ACP_V3_DELETE_PATH) share the remote-address allowlist coverage as their v2 counterparts; all are scoped out of this plan for the same reason as the other ACP routes (runtime requires a live v3 process).
- **test targets** (current, post-cutover names): `api_acp_delete_sessions` (the sole surviving delete endpoint), `_acp_delete_session`'s v3 dispatch branch, `_acp_delete_many`'s v3 dispatch, and `_acp_sessions_for_workspace`'s `include_v3` parameter all have unit coverage in tests/test_web.py: `TestAcpSessionsForWorkspaceV3`, `TestAcpDeleteSessionV3Dispatch`, `TestAcpDeleteManyV3Dispatch` (all three unchanged — permanent SC-3 exceptions, denote store format not engine identity), `TestAcpDeleteEndpointForV3Ids` (renamed from `TestApiAcpV3DeleteSessionsEndpoint`), `TestApiAcpDeleteSessionsEndpointMixedWorkspace` (renamed from `TestApiAcpDeleteSessionsV2RegressionForMixedWorkspace`, rescoped to test a historical v2 file entry coexisting with a live v3 session rather than two live engines). `data_kiro_v3.delete_session` and `_find_v3_session_dir` have their own direct coverage in tests/test_data_kiro_v3.py (unchanged): `TestFindV3SessionDir`, `TestDeleteSessionV3`, `TestLoadSessionsSkipsStagedDeletes`.
