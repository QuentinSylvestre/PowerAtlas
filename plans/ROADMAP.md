...
> naming, and three of them (mode-switcher UI, the D32 orphan-lock item, and the steering-palette
> item) are now tracked directly in `plans/260911_ACP_V3_FOLLOWUP_FEATURES.md`.
...
- **Mode-switcher UI for `/acp`** (letting a user pick `spec`/`quick-spec`/`bug-fix`/`plan`). Would unlock the modes SC-9's permission-handling defends against but doesn't itself build a path to. Now tracked in `plans/260911_ACP_V3_FOLLOWUP_FEATURES.md`. Source: `260908_ACP_V3_PRODUCTION_HARDENING` Scope boundaries, Follow-up Work item 3.

- **Self-orphan-lock suppression (D32).** `presence._scan()`'s own-agent orphan-lock guard reads whatever pid the sole supervisor's `_publish_live()` publishes — since `260911_ACP_V2_TO_V3_ENGINE_CUTOVER` merged the two supervisors into one and left `_publish_live()`'s `pid=0` sentinel unchanged (see that plan's Phase 1 step 8), the method always publishes the sentinel, deliberately, to avoid false liveness signals — so the guard can't achieve genuine self-orphan suppression regardless of provider label. A real fix needs the supervisor's own agent pid published to `presence._acp_live` instead of the sentinel. Now tracked in `plans/260911_ACP_V3_FOLLOWUP_FEATURES.md`. Source: `260908_ACP_V3_PRODUCTION_HARDENING` Phase 1 review findings (Reliability engineer), Follow-up Work item 6.

- **3 steering-type slash-command palette entries have no working trigger in `/acp`.** `architecture-selection`/`quick-spec`/`bug-fix` are excluded from both dropdown buckets by the existing `steering` filter, and unreachable even by typing their name — the incoming-prompt converter has no `resource_link` case and the `resource` case needs inline content, not a URI reference. Would need a client-side synthesis mechanism (e.g. inlining the steering document's own content as prompt text); no working shape found yet. Now tracked in `plans/260911_ACP_V3_FOLLOWUP_FEATURES.md`. Source: `260908_ACP_V3_PRODUCTION_HARDENING` Phase 0 rounds 3-4, Follow-up Work item 8.
