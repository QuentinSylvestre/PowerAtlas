...
**Exit criteria**:
- [ ] `custom-agent`-typed entries (e.g. `kiro-default`) no longer appear in either dropdown bucket, verified by unit test for both `_Supervisor` and `_SupervisorV3`.
- [ ] Live verification (Phase 8): the `/acp-v3` palette no longer offers `kiro-default`; selecting any remaining entry (a skill) behaves as before (regression) with no `-32603` reachable through normal UI interaction.
- [ ] New test(s) per Changes point 4, mirroring the existing `TestSupervisorV3` test shape.
- [ ] `.venv-PowerAtlas\Scripts\pytest` passes.
