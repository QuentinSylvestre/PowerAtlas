...
> **Anchors verified against HEAD**: `147325f` (2026-07-07). NOTE: the repo drifted again during review (HEAD advanced past `147325f`) — `/qdev` MUST re-run `git diff 147325f -- <file>` and re-confirm each anchor before editing. This drift is the exact risk in the harness note below.
...
### 7. Open items
- **SC13 (W1)** semantics: confirm "disabled provider" should hide cards, not just tabs (deterministic once the intended UX is stated; surface to user in `/qdev` if the code doesn't already imply it).
...
- **CSRF/Origin (out of scope, user's call)**: command-executing POST endpoints (`/api/provider/save`, launch endpoints) have no CSRF/Origin check. Mitigated by 127.0.0.1 bind + random port. Surfaced to the user; a follow-up would add an `Origin`/`Host` allowlist.
...
## 1) Current State
All anchors re-verified against HEAD `147325f` (2026-07-07).
- **Launcher** (`launcher.py`): `shlex.split(default_args)` runs at `:150` (non-terminal `kiro-ide` path) and `:180` (terminal path), both **outside** the `try/except OSError` (`:154-167`, `:187-192`) that the "never raises" contract (`:130`) promises → malformed args raise `ValueError` to the caller (L2). cmd fallback joins args unguarded into the `/k` string (`:326-328`); only `cwd` is metachar-checked (`_CMD_METACHAR_RE :227`, applied `:324`) (L1). `_TITLE_UNSAFE_RE = ["'&|]` (`:228`) misses `;$\`` (L6). `_SESSION_ID_RE` has no length bound (`:21,143`) (L5). `_build_template_command` whitespace-splits literal segments (`:250`) (L7).
...
- **Config** (`config.py`): corrupt TOML → silent defaults, no backup (`:45-49`) (C5). Unknown keys dropped on load (`:53-55`) and lost on save (`asdict` `:91`) (C7). `trust_all_tools` migration gated on `not config.provider_settings` (`:76`) → suppressed by any provider entry (C6). Load validates only top-level type + bool (`:56-61`); nested element types unchecked (C4-nested).
- **Data** (`data.py`): module `_cache` (`:15`) is a lockless dict; `discover_workspaces_with_counts` (`:145-179`) is check-then-act with no lock/single-flight (D1). `SessionCache.get` (`:84`) shallow-copies the list but shares mutable `Session` objects (`:19-28`) (D3). Unknown provider fails **open** → all providers (`:162-164`), caching a per-string key (`:154`) (D5); `get_sessions` fails closed (`:188-190`) — inconsistent. `data_kiro.py` has `_tail_cache`/`_first_prompt_cache` (`:254,310`); `data_claude.py` `get_session_tail`/`get_first_prompt` (`:378,426`) have **none** (D2) and return `[]`/`""` on unresolvable cwd (`:380-382,428-430`) (D7). kiro `get_first_prompt` negative-caches `""` for 60s with no mtime guard (`data_kiro.py:316-336`) (D4).
- **Web** (`web.py`): `partials_workspaces` (`:329-403`), `partials_pinned_workspaces` (`:266-326`), `search` (`:406-502`) render cards for **all** available providers; only `/api/available-providers` (`:543`) filters by `enabled` (→ tabs only) (W1). `get_provider_settings` (`:547-551`) returns synthesized `enabled:true` for unknown keys (W4).
- **GUI** (`templates/index.html`): `switchProvider()` swaps `#workspace-cards` innerHTML without calling `updateActionBar()`; the `htmx:afterSwap` handler calls only `loadExpandedCards()` → stale action bar after a provider switch drops selected rows (G1).
- **Lifecycle** (`__main__.py`): three `store_true` flags with no mutually-exclusive group (`:321-334`) → `--stop --restart` silently stop-only (Lc1). `--restart` uses a fixed `time.sleep(0.5)` (`:342-343`) before relaunch → slow shutdown can leave the mutex held and skip the relaunch (H8).
- **Dead code**: `templates/settings.html` and 14 `.settings-*` rules in `static/style.css` are orphaned (the `/settings` route + `POST /api/settings` were removed) (SC24).
...
| W1 hides disabled-provider workspaces + session rows (behaviour change) | Disabled provider's discovery vanishes; explicit pins kept | Scoped decision (cards+badges+expand-rows; pins honored); flagged for user confirm in the report. |
| Security (out of plan scope): no CSRF/Origin check on command-executing POSTs (`/api/provider/save`, launch endpoints) | Localhost drive-by could set `default_args` then launch (mitigated by random port + 127.0.0.1 bind) | Flagged to user; not fixed here. Consider an `Origin`/`Host` allowlist in a follow-up. |
